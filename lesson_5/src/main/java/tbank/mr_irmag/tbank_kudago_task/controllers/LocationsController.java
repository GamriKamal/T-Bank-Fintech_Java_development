...
import tbank.mr_irmag.tbank_kudago_task.component.LocationCaretaker;
...
public class LocationsController {
    ...
    private final LocationCaretaker caretaker = new LocationCaretaker();
    ...
    public ResponseEntity<?> getAllLocations() {
        try {
            ...
            ErrorResponse errorResponse = new ErrorResponse("Error getting locations", e.getMessage());
    ...
    public ResponseEntity<?> createLocation(@RequestBody Location location) {
        try {
            ...
            ErrorResponse errorResponse = new ErrorResponse("Location creation error", e.getMessage());
    ...
    public ResponseEntity<?> updateLocation(@PathVariable("slug") String slug, @RequestBody String name) {
        try {
            ...
            ErrorResponse errorResponse = new ErrorResponse("Location update error", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
        }
    }
    ...
    public ResponseEntity<?> deleteLocation(@PathVariable("slug") String slug) {
        try {
            ...
            ErrorResponse errorResponse = new ErrorResponse("Location deletion error", e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(errorResponse);
        }
    }

    @PostMapping("/save")
    @Operation(summary = "Сохранить текущее состояние локаций", description = "Сохраняет текущее состояние локаций в Memento.")
    public ResponseEntity<?> saveLocationsState() {
        try {
            caretaker.save(locationsService);
            return ResponseEntity.ok("The state of the locations has been successfully saved.");
        } catch (Exception e) {
            ErrorResponse errorResponse = new ErrorResponse("Error saving location status", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(errorResponse);
        }
    }

    @PostMapping("/restore")
    @Operation(summary = "Восстановить предыдущее состояние локаций", description = "Восстанавливает предыдущее состояние локаций из Memento.")
    public ResponseEntity<?> restoreLocationsState() {
        try {
            caretaker.restore(locationsService);
            return ResponseEntity.ok("The state of the locations has been successfully restored.");
        } catch (Exception e) {
            ErrorResponse errorResponse = new ErrorResponse("Error restoring the state of locations", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(errorResponse);
        }
    }
}
