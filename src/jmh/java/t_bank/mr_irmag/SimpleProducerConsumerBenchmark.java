...
public class SimpleProducerConsumerBenchmark {
    ...
    public void tearDown() throws IOException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("SimpleProducerConsumerBenchmark_report.txt", true))) {
            writer.write("Benchmark, Mode, Throughput (ops/s), Latency (ms), Consumer Processing Time (ms), Delivery Ack Time (ms)\n");

            writer.write(String.format("testKafkaProducerConsumer, %s, %.2f, %.2f, %.2f, %.2f\n",
                    Mode.AverageTime,
            ...
            writer.write(String.format("testRabbitMQProducerConsumer, %s, %.2f, %.2f, %.2f, %.2f\n",
                    Mode.AverageTime,
                    (double) totalProducerLatencyRabbit / 1.0,
                    (double) totalConsumerProcessingTimeRabbit / 1.0,
                    0,
                    (double) totalDeliveryAckLatencyRabbit / 1.0));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("MultipleConsumersBenchmark_report.txt", true))) {
            ...
            writer.write(String.format("testKafkaMultipleConsumers, %s, %.2f, %.2f, %.2f, %.2f" + "\n",
                    Mode.AverageTime,
                    (double) totalProducerLatencyKafka / 1,
                    (double) totalConsumerProcessingTimeKafka / 1,
                    0,
                    0));
            ...
            writer.write(String.format("testRabbitMQMultipleConsumers, %s, %.2f, %.2f, %.2f, %.2f" + "\n",
                    ...
                    (double) totalConsumerProcessingTimeRabbit / 1,
                    0,
                    (double) totalDeliveryAckLatencyRabbit / 1));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
