...
public class JwtService {
    ...
    private String generateToken(Map<String, Object> extraClaims, UserDetails userDetails, boolean rememberMe) {
        long expirationTime = rememberMe ? 1000L * 60 * 60 * 24 * 30 : 1000L * 60 * 10; // 30 дней или 10 минут
        return Jwts.builder()
    ...
    private Claims extractAllClaims(String token) {
        try {
            ...
            throw new IllegalArgumentException("Некорректный токен" + e.getLocalizedMessage());
        }
    }
    ...
    private Key getSigningKey() {
        ...
        if (keyBytes.length < 32) {
            throw new IllegalArgumentException("Ключ для подписи должен быть как минимум 32 байта (256 бит).");
        }
